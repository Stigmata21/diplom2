  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        (token as unknown as { rememberMe?: boolean }).rememberMe = (user as unknown as { rememberMe?: boolean }).rememberMe;
      }
      return token;
    },
    async session({ session, token }) {
      if (token) {
        ...
        (session as unknown as { rememberMe?: boolean }).rememberMe = (token as unknown as { rememberMe?: boolean }).rememberMe;
      }
      return session;
    }
  },
